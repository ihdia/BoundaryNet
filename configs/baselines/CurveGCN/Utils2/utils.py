...
from Utils2.poly_point_isect import isect_polygon__naive_check
import copy
import Utils2.dpfuncdist as dputils
import random
...
def prepare_gcn_component(pred_polys, grid_sizes, max_poly_len, n_adj=3):
    ...
    for i in range(pred_polys.shape[0]):
        ...
        for size in grid_sizes:
            ...
            curr_p_grid_size[:, 0] *= size
            curr_p_index = np.sum(curr_p_grid_size, axis=-1)
            # print("currebt_p", curr_p_index)
    ...
    # print(batch_array_feature_indexs)
